...
function NavIconSvg({ name, className }: { name: NavIcon; className?: string }) {
    const c = 'h-5 w-5 shrink-0 ' + (className ?? 'text-wa-muted');
    switch (name) {
        case 'dashboard':
            return (
...
function buildNavItems(): NavItem[] {
    return [
        { label: 'ダッシュボード', route: 'home', icon: 'dashboard' },
        { label: 'KPI・案件', route: 'sales.summary', icon: 'chart', alsoMatch: ['sales.ranking', 'sales.trend'] },
        { label: '売上明細', route: 'sales.records', icon: 'table' },
        ...
        { label: 'ID・パス', route: 'admin.credentials.index', icon: 'key', canKey: 'admin_credentials' },
