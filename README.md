...
**LOM.Hub（LOM Sales Hub）** は、営業チームが毎日使う情報・操作をすべて 1 つの画面に集約した社内 Web ポータルです。

KPI の確認、昼休憩の予約、業務依頼、周知の閲覧、商材情報の参照——これらをバラバラのツールを行き来せず、**`/portal` にアクセスするだけで完結**させることを目指して開発しました。

- Laravel 13 + React（Inertia.js）によるフルスタック構成で、SPA のような快適な操作感を実現
- Discord / KOT（King of Time）と連携し、打刻・通知を自動化
- Docker（Laravel Sail）でローカル環境を統一、本番は Render に自動デプロイ
