...
- **Laravel 13 + Inertia.js（React）**: フルスタック構成でありながら、SPA のようなページ遷移を実現。API を別途設計せず、Laravel のルーティング・認証・バリデーションをそのまま活用しています
- **責務の分離**: Controller は HTTP の入出力のみ、ビジネスロジックは Service 層に集約。React コンポーネントは表示に専念させることで、保守性の高いコードベースを維持しています
- **外部連携の堅牢性**: Discord Webhook・KOT API・GAS との連携はすべてキュージョブで非同期処理し、送信ログと失敗時のリトライ機能を実装しています
- **Docker 対応**: Laravel Sail によるローカル開発環境と、マルチステージビルドによる軽量な本番イメージを構成しています
